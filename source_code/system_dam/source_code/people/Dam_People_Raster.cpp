...
//init static members
Tables *Dam_People_Raster::raster_table=NULL;

//Default constructor
Dam_People_Raster::Dam_People_Raster(void){
	this->element=NULL;
	this->raster_is_set=false;
	this->type=_dam_raster_types::pop_total;

	this->result_affect_pop=0.0;
	this->result_endanger_pop=0.0;

	//count the memory
	Sys_Memory_Count::self()->add_mem(sizeof(Dam_People_Raster), _sys_system_modules::DAM_SYS);
}
//Default destructor
Dam_People_Raster::~Dam_People_Raster(void){
	this->delete_raster_elem_info();
	//count the memory
	Sys_Memory_Count::self()->minus_mem(sizeof(Dam_People_Raster), _sys_system_modules::DAM_SYS);
}
//_____________
//public
//Read in the economic damage data from file: mobile, immobile and individuell stockvalues
void Dam_People_Raster::read_all_damage_raster_per_file(string fname_type, string fname_people_stock){
		//damage function information
		this->set_raster_type(_dam_raster_types::pop_dam_category);
		this->read_damage_raster_per_file(fname_type);
		//allocate the element information
		this->allocate_raster_elem_info();
		this->extract_damagetype2raster_elem(this);
		this->raster_is_set=true;

		Dam_People_Raster buffer;
		buffer.set_raster_type(_dam_raster_types::pop_density);
		buffer.read_damage_raster_per_file(fname_people_stock);
		if(this->compare_raster_infos(&buffer)==false){
			...
			this->extract_people_number2raster_elem(&buffer);
		}
		this->set_raster_type(_dam_raster_types::pop_total);
}
...
void Dam_People_Raster::transfer_input_members2database(QSqlDatabase *ptr_database){
	//mysql query with the table_model
	QSqlTableModel model(0,*ptr_database);

	//the table is set (the name and the column names) and allocated
	try{
		Dam_People_Raster::set_raster_table(ptr_database);
	}
	catch(Error msg){
		throw msg;
	}
	ostringstream cout;
	cout <<"Add the general data of the people2risk raster " << this->name <<" to the database ..."<< endl;
	Sys_Common_Output::output_dam->output_txt(&cout);

	//give the table name to the query model
	model.setTable((Dam_People_Raster::raster_table->get_table_name()).c_str());

	//evaluate the global identifier
	this->number=Dam_People_Raster::raster_table->maximum_int_of_column(Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id),ptr_database)+1;

	//insert row
	model.insertRows(0, 1);

	//insert data
	model.setData(model.index(0,model.record().indexOf((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id)).c_str())),this->number);
	model.setData(model.index(0,model.record().indexOf((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_name_db)).c_str())),this->name.c_str());
	model.setData(model.index(0,model.record().indexOf((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_no_x_db)).c_str())),this->no_elems_x);
	model.setData(model.index(0,model.record().indexOf((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_no_y_db)).c_str())),this->no_elems_y);
	model.setData(model.index(0,model.record().indexOf((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_x_origin_db)).c_str())),this->x_coor_origin);
	model.setData(model.index(0,model.record().indexOf((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_y_origin_db)).c_str())),this->y_coor_origin);
	model.setData(model.index(0,model.record().indexOf((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_cellsize_db)).c_str())),this->width_x);
	model.setData(model.index(0,model.record().indexOf((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_noinfo_db)).c_str())),this->no_info_value);
	...
	if(model.lastError().isValid()){
		...
		ostringstream info;
		info << "Table Name                : " << Dam_People_Raster::raster_table->get_table_name() << endl;
	...
	brows_buff.set_tablename(Dam_People_Raster::raster_table->get_table_name());
	//set a where filter
	ostringstream buff_filter;
	buff_filter<< Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id) << " = " << this->number;
	brows_buff.set_where_filter(buff_filter.str());
	//get the query string
	string str_query_buff;
	str_query_buff =brows_buff.get_inserting_polygon_filter(Dam_People_Raster::raster_table->get_column_name(dam_label::raster_polygon), &this->geometrical_bound);
	...
	int glob_id=Dam_People_Element::get_max_glob_id_elem_table(ptr_database)+1;
	//get the header for the query
	string query_header;
	query_header=Dam_People_Element::get_insert_header_elem_table(ptr_database);
...
//void Dam_People_Raster::input_raster_perdatabase(const QSqlTableModel *results, QSqlDatabase *ptr_database, const int glob_index, const _sys_system_id id){
//	this->number=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id)).c_str()).toInt();
//	this->name=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_name_db)).c_str()).toString().toStdString();
//	this->no_elems_x=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_no_x_db)).c_str()).toInt();
//	this->no_elems_y=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_no_y_db)).c_str()).toInt();
//	this->x_coor_origin=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_x_origin_db)).c_str()).toDouble();
//	this->y_coor_origin=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_y_origin_db)).c_str()).toDouble();
//	this->width_x=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_cellsize_db)).c_str()).toDouble();
//	this->width_y=this->width_x;
//	this->no_info_value=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_noinfo_db)).c_str()).toDouble();
//	this->connected_flag=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_connected)).c_str()).toBool();
//
//	try{
...
//
//		int number_poly=Dam_People_Element::select_relevant_elements_database(&elem_results, ptr_database, id ,this->number);
...
//		for(int i=0; i< this->number_polygons; i++){
//			Dam_Damage_System::check_stop_thread_flag();
//			this->element[i].input_element_perdatabase(&elem_results , i);
//		}
//	}
//	catch(Error msg){
//		ostringstream info;
...
void Dam_People_Raster::input_raster_perdatabase_general_data(const QSqlQueryModel *results, const int glob_index){
	this->number=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id)).c_str()).toInt();
	this->name=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_name_db)).c_str()).toString().toStdString();
	this->no_elems_x=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_no_x_db)).c_str()).toInt();
	this->no_elems_y=results->record(glob_index).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_no_y_db)).c_str()).toInt();
...
void Dam_People_Raster::output_member(void){
	...
	//	cout << " No" <<W(13) << "Type_id" << W(18)<< "Stock_people" << label::person_per_sqm<< endl;
...
void Dam_People_Raster::output_number_fp_connected_elem(ostringstream *cout){
...
void Dam_People_Raster::output_result_damage(void){
	ostringstream prefix;
	...
	cout << " Affected              : " << P(2)<< FORMAT_FIXED_REAL << this->result_affect_pop<< label::person << endl;
	cout << " Endangered            : " <<  this->result_endanger_pop << label::person << endl;
	Sys_Common_Output::output_dam->output_txt(&cout, true);
	Sys_Common_Output::output_dam->rewind_userprefix();
}
//Get the results of the affected people for the raster
double Dam_People_Raster::get_affected_pop_results(void){
	return this->result_affect_pop;
}
//Get the results of the endangered people for the raster
double Dam_People_Raster::get_endangered_pop_results(void){
	return this->result_endanger_pop;
}
//Calculate the people2risk damages for the raster
void Dam_People_Raster::calculate_damages(Dam_Impact_Value_Floodplain *impact_fp, const int number){
	int counter=0;
	int counter_fp_id=0;
...
void Dam_People_Raster::output_results2database(QSqlDatabase *ptr_database,const int bound_sz, const string break_sz, bool *was_output, const bool must_output){
	ostringstream prefix;
	prefix << "RAST_"<<this->number<<"> ";
	Sys_Common_Output::output_dam->set_userprefix(prefix.str());
	...
	ostringstream cout;
	cout << "Transfer the people2risk damage results of raster elements of raster " << this->name << " to database..."<< endl;
	Sys_Common_Output::output_dam->output_txt(&cout);

	//get the global index
	int glob_id=Dam_People_Element::get_max_glob_id_erg_table(ptr_database)+1;
	//get the header for the query
	string query_header;
	query_header=Dam_People_Element::get_insert_header_erg_table(ptr_database);
...
void Dam_People_Raster::connect_dam_function2elems(const int number, Dam_People_Damage_Function *functions){
	//damages
	for(int i=0; i< this->number_polygons; i++){
		Dam_Damage_System::check_stop_thread_flag();
		for(int j=0; j<number; j++){
			if(this->element[i].connect_people2risk_type2damage_function(this->get_no_info_asint(), &functions[j])==true){
				break;
			}
		}
	}
...
void Dam_People_Raster::check_damage_function_connection(void){
	QList<int> list_id;
	...
	for(int i=0; i<this->number_polygons; i++){
		Dam_Damage_System::check_stop_thread_flag();
		if(this->element[i].people2risk_type_is_found()==false){
			if(list_id.contains(this->element[i].get_id_people2risk_type())==false){
				list_id.append(this->element[i].get_id_people2risk_type());
				list_id_number.append(1);
			}
			else{
				id=list_id.indexOf(this->element[i].get_id_people2risk_type());
				list_id_number.replace(id,list_id_number.at(id)+1);
			}
			this->element[i].set_noinfo_elem_type();
		}
	}
	if(list_id.length()>0){
		...
		info << "Following indices of people2risk categories are not found"<<endl;
...
void Dam_People_Raster::set_intercepted_elem2reduced_area(Dam_People_Raster *raster, QSqlDatabase *ptr_database, const _sys_system_id id){
	//no interception
...
void Dam_People_Raster::set_intercepted_elem2floodplain_id(Hyd_Model_Floodplain *fp_model){
	...
	for(int i=0; i< this->number_polygons; i++){
		...
		if(polys->check_point_inside(&this->element[i].mid_point)==true){
			...
			if(current_id.count()==0 && new_id.count()==0){
				this->element[i].set_noinfo_elem_type();
				this->element[i].set_id_people2risk_type(this->no_info_value);
				...
				Dam_People_Element::switch_applied_flag_elem_table(ptr_database, base, this->element[i].get_element_index(), this->number, false);
			}
			//switch all to one id
			else if(current_id.count()==0 && new_id.count()!=0){
				this->element[i].set_id_people2risk_type(new_id.at(0));
				this->element[i].transfer_input_members2database(ptr_database, this->number, this->width_x, this->x_coor_origin, this->y_coor_origin);
				Dam_People_Element::switch_applied_flag_elem_table(ptr_database, base, this->element[i].get_element_index(), this->number, false);
			}
			//switch one id tzo another one
				...
				for(int j=0; j<current_id.count(); j++){
					if(this->element[i].get_id_people2risk_type()==current_id.at(j)){
						if(j>=new_id.count()){
							//set noinfo
							this->element[i].set_noinfo_elem_type();
							...
							this->element[i].set_id_people2risk_type(new_id.at(j));
							transfer_required=true;
						}
					}
				}
...
void Dam_People_Raster::transfer_intercepted_elem_data2database(QSqlDatabase *ptr_database){
...
void Dam_People_Raster::create_raster_table(QSqlDatabase *ptr_database){
		if(Dam_People_Raster::raster_table==NULL){
			ostringstream cout;
			cout << "Create poeple2risk raster data database table..." << endl ;
			Sys_Common_Output::output_dam->output_txt(&cout);
			//make specific input for this class
			const string tab_name=dam_label::tab_pop_raster;
			...
			try{
				Dam_People_Raster::raster_table= new Tables();
				if(Dam_People_Raster::raster_table->create_non_existing_tables(tab_name, tab_col, num_col,ptr_database, _sys_table_type::dam)==false){
				...
				Error msg;
				msg.set_msg("Dam_People_Raster::create_raster_table(QSqlDatabase *ptr_database)","Can not allocate the memory", "Check the memory", 10, false);
				ostringstream info;
				info<< "Info bad alloc: " << t.what() << endl;
				msg.make_second_info(info.str());
				throw msg;
...
void Dam_People_Raster::set_raster_table(QSqlDatabase *ptr_database){
	if(Dam_People_Raster::raster_table==NULL){
		//make specific input for this class
		const string tab_id_name=dam_label::tab_pop_raster;
		string tab_col[11];
		...
		try{
			Dam_People_Raster::raster_table= new Tables(tab_id_name, tab_col, sizeof(tab_col)/sizeof(tab_col[0]));
			Dam_People_Raster::raster_table->set_name(ptr_database, _sys_table_type::dam);
		}
		catch(bad_alloc& t){
			Error msg;
			msg.set_msg("Dam_People_Raster::set_raster_table(QSqlDatabase *ptr_database)","Can not allocate the memory", "Check the memory", 10, false);
			ostringstream info;
			info<< "Info bad alloc: " << t.what() << endl;
			msg.make_second_info(info.str());
			throw msg;
...
void Dam_People_Raster::delete_data_in_raster_table(QSqlDatabase *ptr_database){
	//the table is set (the name and the column names) and allocated
	try{
		Dam_People_Raster::set_raster_table(ptr_database);
	}
	...
	//delete the table
	Dam_People_Raster::raster_table->delete_data_in_table(ptr_database);
}
...
void Dam_People_Raster::delete_data_in_raster_table(QSqlDatabase *ptr_database, const int id){
	...
	//delete
	test_filter<< "DELETE ";
	test_filter << " FROM " << Dam_People_Raster::raster_table->get_table_name();
	test_filter << " WHERE ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id) << " = " << id;
	...
	if(results.lastError().isValid()){
		Error msg;
		msg.set_msg("Dam_People_Raster::delete_data_in_raster_table(QSqlDatabase *ptr_database, const int id)","Invalid database request", "Check the database", 2, false);
		ostringstream info;
		info << "Table Name      : " << Dam_People_Raster::raster_table->get_table_name() << endl;
...
void Dam_People_Raster::close_table(void){
	if(Dam_People_Raster::raster_table!=NULL){
		delete Dam_People_Raster::raster_table;
		Dam_People_Raster::raster_table=NULL;
	}
}
//Check if all people2risk raster are connected to the hydraulic system (static)
bool Dam_People_Raster::check_all_raster_connected2hyd(QSqlDatabase *ptr_database){
	QSqlQueryModel results;
	int number=0;
	number=Dam_People_Raster::select_relevant_raster_database(&results, ptr_database, false);

	bool connected=false;
	for(int i=0; i<number; i++){
		connected=false;
		connected=results.record(i).value((Dam_People_Raster::raster_table->get_column_name(dam_label::raster_connected)).c_str()).toBool();
		if(connected==false){
			return false;
		}
	}
...
void Dam_People_Raster::reset_hydraulic_connection_flag(QSqlDatabase *ptr_database){
	QSqlQuery elem_results(0,*ptr_database);
	...
	bool buffer=false;
	filter << "UPDATE " << Dam_People_Raster::raster_table->get_table_name().c_str();
	filter << " SET ";
	filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_connected).c_str() << " = " << functions::convert_boolean2string(buffer) << "  ";

	Data_Base::database_request(&elem_results, filter.str(), ptr_database);
}
//Reset the hydraulic connection in elements (static)
void Dam_People_Raster::reset_hydraulic_connection_elements(QSqlDatabase *ptr_database){
	QSqlQuery elem_results(0,*ptr_database);
	ostringstream filter;
	try{
		Dam_People_Element::set_elem_table(ptr_database);
	...
	filter << " SET ";
	filter << Dam_People_Element::elem_table->get_column_name(dam_label::conn_fp_id).c_str() << " = -1 , ";
	filter << Dam_People_Element::elem_table->get_column_name(dam_label::conn_fp_elem_id).c_str() << " = -1 ";
	filter << " where " <<Dam_People_Element::elem_table->get_column_name(dam_label::conn_fp_id).c_str() << " > " << -1 ;

	Data_Base::database_request(&elem_results, filter.str(), ptr_database);
}
//Select and count the number of relevant damage raster in a database table (static)
int Dam_People_Raster::select_relevant_raster_database(QSqlQueryModel *results, QSqlDatabase *ptr_database, const bool with_output){
	int number=0;
	try{
		Dam_People_Raster::set_raster_table(ptr_database);
	}
	...
	if(with_output==true){
		ostringstream cout;
		cout << "Search for relevant people2risk raster in database..."<< endl ;
		Sys_Common_Output::output_dam->output_txt(&cout);
	}

	ostringstream test_filter;
	test_filter<< "Select ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id) << " , ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_name_db) << " , ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_no_x_db) << " , ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_no_y_db) << " , ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_x_origin_db) << " , ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_y_origin_db) << " , ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_cellsize_db) << " , ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_connected) << " , ";
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_noinfo_db);
	test_filter << " from " << Dam_People_Raster::raster_table->get_table_name();
	test_filter << " where " <<Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id) << ">= 0";
	test_filter << " order by " << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id);
	...
	if(results->lastError().isValid()){
		Error msg;
		msg.set_msg("Dam_People_Raster::select_relevant_raster_database(QSqlTableModel *results,  const bool with_output)","Invalid database request", "Check the database", 2, false);
		ostringstream info;
		info << "Table Name      : " << Dam_People_Raster::raster_table->get_table_name() << endl;
		info << "Table error info: " << results->lastError().text().toStdString() << endl;
		msg.make_second_info(info.str());
	...
	if(with_output==true){
		ostringstream cout;
		cout << number << " relevant people2risk raster(s) are found in database" << endl ;
		Sys_Common_Output::output_dam->output_txt(&cout);
	}
...
int Dam_People_Raster::select_relevant_raster_database(QSqlQueryModel *results, QSqlDatabase *ptr_database, const int raster_no){
	int number=0;
	try{
		Dam_People_Raster::set_raster_table(ptr_database);
	}
	...
	test_filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_noinfo_db);
	test_filter << " from " << Dam_People_Raster::raster_table->get_table_name();
	test_filter << " where " <<Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id) << "= "<<raster_no;
	test_filter << " order by " << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id);
	...
	if(results->lastError().isValid()){
		Error msg;
		msg.set_msg("Dam_People_Raster::select_relevant_raster_database(QSqlTableModel *results,  const int raster_no)","Invalid database request", "Check the database", 2, false);
		ostringstream info;
		info << "Table Name      : " << Dam_People_Raster::raster_table->get_table_name() << endl;
		info << "Table error info: " << results->lastError().text().toStdString() << endl;
		msg.make_second_info(info.str());
...
void Dam_People_Raster::sum_total_results(QSqlDatabase *ptr_database, const _sys_system_id id, const int bound_sz, const string break_sz){
	try{
	...
	ostringstream query;
	query << "SELECT ";
	query << Dam_People_Element::elem_erg_table->get_column_name(dam_label::pop_affected)<<" , "  ;
	query << Dam_People_Element::elem_erg_table->get_column_name(dam_label::pop_endangered);
	query <<" FROM "<< Dam_People_Element::elem_erg_table->get_table_name();
	query << " WHERE ";
	query << Dam_People_Element::elem_erg_table->get_column_name(label::areastate_id) <<" = " << id.area_state<< " AND ";
	query << Dam_People_Element::elem_erg_table->get_column_name(label::measure_id) <<" = " << id.measure_nr << " AND ";
	query << Dam_People_Element::elem_erg_table->get_column_name(hyd_label::sz_bound_id) <<" = " << bound_sz <<" AND ";
	query << Dam_People_Element::elem_erg_table->get_column_name(risk_label::sz_break_id) <<" = '" << break_sz << "' AND ";
	query << Dam_People_Element::elem_erg_table->get_column_name(dam_label::raster_id) <<" = " << this->number;
	...
	for(int i =0; i< results.rowCount(); i++){
		this->result_affect_pop=this->result_affect_pop+results.record(i).value((Dam_People_Element::elem_erg_table->get_column_name(dam_label::pop_affected)).c_str()).toDouble();
		this->result_endanger_pop=this->result_endanger_pop+results.record(i).value((Dam_People_Element::elem_erg_table->get_column_name(dam_label::pop_endangered)).c_str()).toDouble();
	}
}
//_________
//private
//Set the area of an element to the given area
void Dam_People_Raster::set_area_element2given(const int index, const double area){
...
void Dam_People_Raster::allocate_raster_elem_info(void){
	if(this->element==NULL){
		try{
			this->element=new Dam_People_Element[this->number_polygons];
		}
		catch(bad_alloc &t){
...
void Dam_People_Raster::delete_raster_elem_info(void){
	if(this->element!=NULL){
		delete []this->element;
		this->element=NULL;
	}
...
void Dam_People_Raster::extract_damagetype2raster_elem(Dam_Raster *raster){
	for(int i=0; i< this->number_polygons;i++){
		this->element[i].set_id_people2risk_type(raster->get_characteristics_asint(i));
...
void Dam_People_Raster::extract_people_number2raster_elem(Dam_Raster *raster){
	for(int i=0; i< this->number_polygons;i++){
		this->element[i].set_indiv_pop_stock_value(raster->get_characteristics_asdouble(i));
		if(raster->get_characteristics_asint(i)==raster->get_no_info_asint()){
			this->element[i].set_noinfo_elem_density();
		}
	}
}
//Transfer the midpoint information from the raster element to the raster element information
void Dam_People_Raster::transfer_midpoint2elem_info(void){
//	for(int i=0; i<this->number_polygons;i++){
...
void Dam_People_Raster::set_sys_system_id(const _sys_system_id id){
	for(int i=0; i<this->number_polygons;i++){
		this->element[i].set_systemid(id);
	}
}
//Count the number of the floodplain connected elements
int Dam_People_Raster::count_number_fp_connected_elem(void){
	int count=0;
	for(int i=0; i< this->number_polygons; i++){
		if(this->element[i].get_index_floodplain()>=0){
			count++;
...
void Dam_People_Raster::set_connection_flag2database(const bool flag, QSqlDatabase *ptr_database){
	...
	filter << "UPDATE " << Dam_People_Raster::raster_table->get_table_name().c_str();
	filter << " SET ";
	filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_connected).c_str() << " = " << functions::convert_boolean2string(this->connected_flag) << "  ";
	filter << " WHERE " ;
	filter << Dam_People_Raster::raster_table->get_column_name(dam_label::raster_id) << " = " << this->number;
...
//Set error(s)
Error Dam_People_Raster::set_error(const int err_type){
	string place="Dam_People_Raster::";
	string help;
	string reason;
	int type=0;
	bool fatal=false;
...
//Set warning(s)
Warning Dam_People_Raster::set_warning(const int warn_type){
	string place="Dam_People_Raster::";
	string help;
	string reason;
	string reaction;
	...
	switch (warn_type){
		case 0://damage function is not found
			place.append("check_damage_function_connection");
		...
		case 1://the raster are intercpeting each other
			place.append("set_intercepted_elem2reduced_area(Dam_People_Raster *raster)");
