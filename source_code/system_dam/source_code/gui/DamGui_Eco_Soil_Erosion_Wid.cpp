...
//Default constructor
DamGui_Eco_Soil_Erosion_Wid::DamGui_Eco_Soil_Erosion_Wid(DataRole role, QWidget *parent) : _Sys_Abstract_Base_Wid(parent) {
	// Qt Stuff
	...
	this->set_headLabel_text("Soil Erosion Function");

	//gui
	ui.id->set_label_text("Id");
	ui.id->set_tooltip("Id of the soil erosion type");
	ui.id->set_range(0,999999);
	ui.name->set_label_text("Name");
	ui.name->set_tooltip("Name of the soil erosion type");
	...
	//count the memory
	Sys_Memory_Count::self()->add_mem(sizeof(DamGui_Eco_Soil_Erosion_Wid), _sys_system_modules::DAM_SYS);
}
//Default destructor
DamGui_Eco_Soil_Erosion_Wid::~DamGui_Eco_Soil_Erosion_Wid(void)  {
	Sys_Memory_Count::self()->minus_mem(sizeof(DamGui_Eco_Soil_Erosion_Wid), _sys_system_modules::DAM_SYS);
}
//______________
//public methods
//method to set the whole widget editable
void DamGui_Eco_Soil_Erosion_Wid::set_editable(bool state) {
...
void DamGui_Eco_Soil_Erosion_Wid::set_member(_Sys_Abstract_Base_Wid *ptr) {
	//upcast to derived object
	DamGui_Eco_Soil_Erosion_Wid *other = (DamGui_Eco_Soil_Erosion_Wid *) ptr;
	if (other == NULL) {
...
//Set default values
void DamGui_Eco_Soil_Erosion_Wid::set_default_values(void) {
	//head widget
...
void DamGui_Eco_Soil_Erosion_Wid::set_member(QSqlDatabase *ptr_database, const int func_no){
	try{	
		QSqlTableModel results(0, *ptr_database);
		this->ptr_database=ptr_database;

		if(Dam_Eco_Soil_Erosion_Function::select_relevant_functions_database(&results, func_no)==0){
			this->set_headLabel_text("Unknown Soil-Erosion Function");
			this->setEnabled(false);
		}

		Dam_Eco_Soil_Erosion_Function buffer;
		...
		this->ui.widget_plot->getPlotPtr()->set_axis_title("Velocity [m/s]", "Soil erosion factor [-]");
...
void DamGui_Eco_Soil_Erosion_Wid::show_as_dialog(void) {
	Sys_Base_Edit_Dia dialog(NULL, this);
	DamGui_Eco_Soil_Erosion_Wid inDiaWid(DataRole::EditRole, &dialog);
