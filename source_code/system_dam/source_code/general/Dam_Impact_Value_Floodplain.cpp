...
void Dam_Impact_Value_Floodplain::set_impact_values_database(const int bound_sz, const string break_sz, QSqlDatabase *ptr_database, const _sys_system_id sys_id){
	...
	// ==========================================
	// HIER DIE ÄNDERUNG: STREAMING STATT PAGING
	// ==========================================

	// 1. Eine normale QSqlQuery für den Datenstrom vorbereiten
	QSqlQuery query(*ptr_database);

	// 2. Die neue Stream-Methode einmalig ausführen (lädt das gesamte Ergebnis am Stück)
	Hyd_Element_Floodplain::select_all_relevant_results_elements_stream(&query, ptr_database, sys_id, this->index_floodplain, bound_sz, break_sz, false);

	// 3. Durch den Stream wandern und Werte ohne Offset/Limit-Verzögerung einlesen
	while (query.next()) {
		// Index 0 MUSS die Spalte 'hyd_label::elemdata_id' in Ihrer neuen SELECT-Abfrage sein!
		int elem_id = query.value(0).toInt();

		// Daten direkt aus der aktuellen Zeile des Streams in das Element einlesen
		this->impact_values[elem_id].readout_data_from_query(&query);
	}
