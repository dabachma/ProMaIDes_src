...
//Default constructor
Fpl_Dike_Geo_Hinterland::Fpl_Dike_Geo_Hinterland(void){
	...
	Sys_Memory_Count::self()->add_mem(sizeof(Fpl_Dike_Geo_Hinterland), _sys_system_modules::FPL_SYS);//count the memory
}
//Default destructor
Fpl_Dike_Geo_Hinterland::~Fpl_Dike_Geo_Hinterland(void){

	this->delete_segments();
	Sys_Memory_Count::self()->minus_mem(sizeof(Fpl_Dike_Geo_Hinterland), _sys_system_modules::FPL_SYS);//count the memory
}
//__________
//public
//Set the geometrie of the the hinterland geometry per database table
void Fpl_Dike_Geo_Hinterland::set_segments(Fpl_Section_Points *base_point, QSqlDatabase *ptr_database, const _sys_system_id id, const int section_id){
	//output
	ostringstream cout;
	cout <<"Set the hinterland geometry of the dike... "  << endl ;
	Sys_Common_Output::output_fpl->output_txt(&cout,false);

	QSqlQueryModel results;

	Fpl_Section_Points::select_different_zone_points(&results, id, section_id, fpl_label::hinterland , ptr_database, fpl_label::region_surround, fpl_label::sec_dike);
	...
	try{
		if(this->number_segments==1){
			//just one segment
			Fpl_Section_Points next(fpl_label::hinterland);
			next.set_point_per_database(&results,0);
			//begin with the base, end with the first point; no points between
			this->ptr_segments[0].set_points(base_point, &next);
		}
		else{
			//more than one segment
			Fpl_Section_Points next(fpl_label::hinterland);
			Fpl_Section_Points before(fpl_label::hinterland);
			for(int i=0; i< this->number_segments; i++){
				if(i==0){//just read in points
					next.set_point_per_database(&results,i);
					this->ptr_segments[i].set_points(base_point, &next);
					before=next;
				}
				else{
					next.set_point_per_database(&results,i);
					this->ptr_segments[i].set_points(&before,&next);
					before=next;
				}
			}
		}
...
void Fpl_Dike_Geo_Hinterland::set_segments(Fpl_Section_Points *base_point,  QFile *ifile, int *line_counter){
	//read in
	string myline;
	QString qmyline;
	...
	do{
		...
		if(myline==fpl_label::end_hinterland){
			break;
		}
	...
	try{
		...
		do{
			...
			if(myline==fpl_label::end_hinterland){
				...
				if(counter_point==0 ){
					this->ptr_segments[counter_point].set_points(base_point, &next );
					before=next;
				}
				else{
		...
		while(ifile->atEnd()==false && counter_point<this->number_points-1);
...
void Fpl_Dike_Geo_Hinterland::transfer_geometry2database(const int section_id, const _sys_system_id id, QSqlDatabase *ptr_database){
	Fpl_Section_Points buffer;
	for(int i=0; i<this->number_segments; i++){
		buffer.set_point(this->ptr_segments[i].point2);
		buffer.set_zone_number(0);
		buffer.set_point_name(fpl_label::hinterland);
...
void Fpl_Dike_Geo_Hinterland::output_geometrie(void){
	ostringstream cout;
	cout<<"HINTERLAND GEOMETRY"<< endl;
...
int Fpl_Dike_Geo_Hinterland::get_number_segments(void){
	return this->number_segments;
}
//Get a pointer to a segments specified with an index
Fpl_Dike_Segment *Fpl_Dike_Geo_Hinterland::get_segment(const int index){
...
void Fpl_Dike_Geo_Hinterland::allocate_segments(void){
	try{
...
void Fpl_Dike_Geo_Hinterland::delete_segments(void){
...
void Fpl_Dike_Geo_Hinterland::calc_total_length(void){
	this->total_length=0.0;
	this->total_horizontal=0.0;
	this->total_vertical=0.0;
...
void Fpl_Dike_Geo_Hinterland::check_cubature(void){
	
}
//Find the key-values for the file input of the geometrical points
void Fpl_Dike_Geo_Hinterland::find_key_values_file(const string myline){
	int pos=-1;
	string buffer;
...
//Set error(s)
Error Fpl_Dike_Geo_Hinterland::set_error(const int err_type){
		string place="Fpl_Dike_Geo_Hinterland::";
		string help;
		string reason;
		int type=0;
		bool fatal=false;
	...
	switch (err_type){
		...
		case 2://wrong input
			...
			help="Check the information of the hinterland geometry in file";
			type=1;
			break;
...
//Set warning(s)
Warning Fpl_Dike_Geo_Hinterland::set_warning(const int warn_type){
	string place="Fpl_Dike_Geo_Hinterland::";
		string help;
		string reason;
		string reaction;
