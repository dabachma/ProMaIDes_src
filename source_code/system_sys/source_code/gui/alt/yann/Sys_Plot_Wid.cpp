...
void Sys_Plot_Wid::set_curve_characteristics(const int index, const QwtText title, const QwtPlotCurve::CurveStyle style, const QColor color, const int width)
{
	if ((0 > index) || (index >= this->number_curve)) return;
	...
	this->my_color = color;
	this->my_width = width;
	this->curves[index].setPen(QPen(this->my_color, this->my_width));
}
//Set curve data
...
void Sys_Plot_Wid::set_curve_symbols(const int index, const int size, QwtSymbol::Style style, const QColor color)
{
	if ((0 > index) || (index >= this->number_curve)) return;
	this->symbol[index].setStyle(style);
	this->symbol[index].setPen(color);
	this->symbol[index].setSize(size);
	this->curves[index].setSymbol(this->symbol[index]);
}
//Set the legend
...
void Sys_Plot_Wid::updateMyLegend(QwtLegend* legend, bool update) const
{
	if (this->legend == NULL)
		return;
...
void Sys_Plot_Wid::editPlot(void)
{
	...
	for (QwtPlotItemIterator it = itmList.begin(); it != itmList.end(); ++it) {
		if ((*it)->rtti() == QwtPlotItem::Rtti_PlotCurve) {
			if ((*it)->isVisible()) {
				...
				int i = itmList.indexOf(c) - 1;
				if (i != -1) {
		/*for (int i = 0; i < this->number_curve; ++i) {
			if (this->curves[i].isVisible()) {*/
					Sys_CurveSettings_Wid *curveSettings = new Sys_CurveSettings_Wid();
					PlotSettings->ui.tabWidget->addTab(curveSettings, this->curves[i].title().text());

					QwtText lastCurveTitle = this->curves[i].title().text();
					QwtPlotCurve::CurveStyle lastCurveStyle = this->curves[i].style();
					QColor lastCurveColor = this->curves[i].pen().color();
					curveSettings->curveColor = lastCurveColor;
					int lastCurveWidth = this->curves[i].pen().width();

					curveSettings->ui.curveTitleLineEdit->setText(lastCurveTitle.text());
					curveSettings->ui.curveStyleComboBox->setCurrentIndex(lastCurveStyle);
					curveSettings->ui.curveWidthSpinBox->setValue(lastCurveWidth);
					this->set_curve_characteristics(i, lastCurveTitle, lastCurveStyle, lastCurveColor, lastCurveWidth);

					int lastSymbolSize = this->curves[i].symbol().size().width();
					QwtSymbol::Style lastSymbolStyle = this->curves[i].symbol().style();
					QColor lastSymbolColor = this->curves[i].symbol().pen().color();
					curveSettings->symbolColor = lastSymbolColor;

					curveSettings->ui.symbolStyleComboBox->setCurrentIndex(lastSymbolStyle + 1);
					curveSettings->ui.symbolSizeSpinBox->setValue(lastSymbolSize);
					this->set_curve_symbols(i, lastSymbolSize, lastSymbolStyle, this->curves[i].symbol().pen().color());
					this->updateMyLegend(this->legend, true);
				}
			}
		}
	}
	...
	if (PlotSettings->start_dialog()) {
		...
		this->grid->enableX(PlotSettings->isVerticalGridlinesEnabled());
		this->grid->enableY(PlotSettings->isHorizontalGridlinesEnabled());

		//Set curve parameters
		for (int i = 0; i < PlotSettings->ui.tabWidget->count() - 1; ++i) {
			...
			this->set_curve_characteristics(i, widget->getCurveTitle(), widget->getCurveStyle(), widget->getCurveColor(), widget->getCurveWidth());
		...
		if (PlotSettings->ui.bottomLegendRadioButton->isChecked() || PlotSettings->ui.topLegendRadioButton->isChecked()) {
			...
			this->updateMyLegend(this->legend, false);
			this->insertLegend(this->legend, PlotSettings->getLegendPosition());
		} else {
			this->updateMyLegend(this->legend, false);
			this->insertLegend(this->legend, PlotSettings->getLegendPosition());
