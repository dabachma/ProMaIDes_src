...
//Default constructor
Hyd_Parse_Coupling_FP2CO_Dikebreak::Hyd_Parse_Coupling_FP2CO_Dikebreak(void){
	this->params.start_height=0.0;
	this->params.max_breach_width=0.0;
	this->params.id_related_point=-1;
	this->params.distance2related=0.0;

	this->found_location=false;

	//count the memory
	Sys_Memory_Count::self()->add_mem(sizeof(Hyd_Parse_Coupling_FP2CO_Dikebreak), _sys_system_modules::HYD_SYS);
...
Hyd_Parse_Coupling_FP2CO_Dikebreak::~Hyd_Parse_Coupling_FP2CO_Dikebreak(void){
	...
	//count the memory
	Sys_Memory_Count::self()->minus_mem(sizeof(Hyd_Parse_Coupling_FP2CO_Dikebreak), _sys_system_modules::HYD_SYS);
...
void Hyd_Parse_Coupling_FP2CO_Dikebreak::parse_params(const int index){
	...
	while (this->GetLine(Command)){
		...
		if(Key == eBEGINFP2COBREAKCOUPLING){
			int number=-1;
			buffer >> number;
			if(buffer.fail()==true){
				ostringstream info;
...
_hyd_fp2co_breach_start_parameters Hyd_Parse_Coupling_FP2CO_Dikebreak::get_params(void){
	return this->params;
}
//_________
//private
//Parse for the location of the breach in the coast-line 
void Hyd_Parse_Coupling_FP2CO_Dikebreak::parse_location_parameter(_hyd_keyword_file Key, word Command){
	int must_found=0;
	...
	do{
		...
		buffer<<Command;
		if (Key == ePOINT_NO){
			buffer>>this->params.id_related_point;
			must_found++;
		}
		else if(Key== eDISTANCE){
			buffer>>this->params.distance2related;
			must_found++;
		}
		else if(Key == eSTART_WATERH){
			buffer>>this->params.start_height;
			must_found++;
		}
...
void Hyd_Parse_Coupling_FP2CO_Dikebreak::check_parameters_found(void){
...
//Set error(s)
Error Hyd_Parse_Coupling_FP2CO_Dikebreak::set_error(const int err_type){
	string place="Hyd_Parse_Coupling_FP2CO_Dikebreak::";
	string help;
	string reason;
	int type=0;
	bool fatal=false;
	...
	switch (err_type){
		case 0://wrong input
			...
			help="Check the FP2CO-dikebreak coupling in file";
			info << "Filename: " << this->input_file_name <<endl;
			info << "Error occurs near line: "<< this->line_counter << endl;
			type=1;
			break;
		case 1://No FP2CO dikebreak coupling in file coupling is found to the given index
			place.append("parse_params(const int index)");
			reason="There are no data corresponding to the given index of FP2CO-dikebreak coupling in file";
