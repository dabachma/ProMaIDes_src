...
void output_csptr( csptr csmat){
    int i,j;
    for(i=0; i<csmat->n;i++){
       for(j=0; j<csmat->nzcount[i];j++){
           printf("%f ", csmat->ma[i][j]);
           printf("%i %i | ", i, csmat->ja[i][j]);
...
void output_vec( double *a, int n){
    printf("--------Vector-----------\n");
    int i;
    for(i=0; i<n;i++){
        printf("%i %f \n", i, a[i]);
...
int lusolve( double *y, double *x, iluptr lu )
{
/*----------------------------------------------------------------------
