...
//use optimized jacobi-matrix and ilu; Changed Daniel 20.10.2010
void AddIdentity_ilu(DlsMat A)
{
  int i;
  ...
  case SUNDIALS_BAND:
    //for (i=0; i<A->M; i++) A->cols[i][A->s_mu] += ONE;
      ...
      for (i=0; i<A->M; i++) A->cols[i][2] += ONE;
    break;
...
void PrintMat(DlsMat A)
{
  ...
  case SUNDIALS_BAND:
    ...
    for (i=0; i < A->N; i++) {
      ...
      //for (j=0; j < start; j++) printf("%2s  ",""); //printf("%12s  ","");
      for (j=start; j <= finish; j++) {
#if defined(SUNDIALS_EXTENDED_PRECISION)
        printf("%12Lg  ", a[j][i-j+A->s_mu]);
#elif defined(SUNDIALS_DOUBLE_PRECISION)
        //printf("%12lg  ", a[j][i-j+A->s_mu]);
		  printf("%1lg  ", a[j][i-j+A->s_mu]);
#else
