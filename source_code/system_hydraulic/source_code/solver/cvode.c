...
//the counters are not reinit
int my_CVodeReInit(void *cvode_mem,  realtype t0, N_Vector y0){
   CVodeMem cv_mem;
  int i,k;
...
//Copy relevant solver data
int my_CVodeCopy(void *memrec_src, void *memrec_dest){
	CVodeMem cv_mem_src;
	CVodeMem cv_mem_dest;
	int i,k;

	/* Check cvode_mem */
	if (memrec_src==NULL) {
		CVProcessError(NULL, CV_MEM_NULL, "CVODE", "my_CVodeCopy", MSGCV_NO_MEM);
		return(CV_MEM_NULL);
	}
	cv_mem_src = (CVodeMem) memrec_src;

	if (memrec_dest==NULL) {
		CVProcessError(NULL, CV_MEM_NULL, "CVODE", "my_CVodeCopy", MSGCV_NO_MEM);
		return(CV_MEM_NULL);
	}
	cv_mem_dest = (CVodeMem) memrec_dest;

	/* Copy step parameters */
	cv_mem_dest->cv_q      = cv_mem_src->cv_q ;
	cv_mem_dest->cv_qprime =cv_mem_dest->cv_qprime;                                       
	cv_mem_dest->cv_next_q=cv_mem_dest->cv_next_q; 

	cv_mem_dest->cv_L      = cv_mem_src->cv_L;
	cv_mem_dest->cv_qwait  = cv_mem_src->cv_qwait;
	cv_mem_dest->cv_etamax = cv_mem_src->cv_etamax;

	cv_mem_dest->cv_qu    = cv_mem_src->cv_qu;
	cv_mem_dest->cv_hu    = cv_mem_src->cv_hu;
	cv_mem_dest->cv_tolsf = cv_mem_src->cv_tolsf;

	cv_mem_dest->cv_hscale = cv_mem_src->cv_hscale; 
	cv_mem_dest->cv_hprime = cv_mem_src->cv_hprime;


	/* Copy zn[0] in the history array */
	for(i=0; i<=cv_mem_src->cv_qmax; i++){
		VCopy_Serial(cv_mem_src->cv_zn[i], cv_mem_dest->cv_zn[i]);
	}
	
	VCopy_Serial(cv_mem_src->cv_acor, cv_mem_dest->cv_acor);
	VCopy_Serial(cv_mem_src->cv_ewt, cv_mem_dest->cv_ewt);
	cv_mem_dest->cv_indx_acor= cv_mem_src->cv_indx_acor;
	

	/* Copy other integrator optional outputs */
	cv_mem_dest->cv_h0u      = cv_mem_src->cv_h0u ;
	cv_mem_dest->cv_h      = cv_mem_src->cv_h ;
	cv_mem_dest->cv_eta =cv_mem_src->cv_eta ;
	cv_mem_dest->cv_next_h   = cv_mem_src->cv_next_h;
	cv_mem_dest->cv_next_q   = cv_mem_src->cv_next_q;

	cv_mem_dest->cv_jcur=cv_mem_src->cv_jcur;

	cv_mem_dest->cv_tn= cv_mem_src->cv_tn;             
	cv_mem_dest->cv_tretlast= cv_mem_src->cv_tretlast;

	for(i=0; i<=cv_mem_src->cv_qmax; i++){
		cv_mem_dest->cv_tau[i]=cv_mem_src->cv_tau[i];
	}
	for(i=0; i<cv_mem_src->cv_qmax; i++){
		cv_mem_dest->cv_l[i]=cv_mem_src->cv_l[i];
	}
	for(i=0; i<=cv_mem_src->cv_maxnef; i++){
		cv_mem_dest->cv_tq[i]=cv_mem_src->cv_tq[i];
	}


	cv_mem_dest->cv_gamma= cv_mem_src->cv_gamma;             
	cv_mem_dest->cv_gamma= cv_mem_src->cv_gamma;            
	cv_mem_dest->cv_gammap= cv_mem_src->cv_gammap;         
	cv_mem_dest->cv_gamrat= cv_mem_src->cv_gamrat;          

	cv_mem_dest->cv_acnrm= cv_mem_src->cv_acnrm;           
	cv_mem_dest->cv_nlscoef= cv_mem_src->cv_nlscoef;   

	cv_mem_dest->cv_saved_tq5=cv_mem_src->cv_saved_tq5;

	/* Copy Stablilty Limit Detection data */

	cv_mem_dest->cv_nor = cv_mem_src->cv_nor;
	for (i = 1; i <= 5; i++)
	for (k = 1; k <= 3; k++) 
	  cv_mem_dest->cv_ssdat[i-1][k-1] = cv_mem_src->cv_ssdat[i-1][k-1];

	/* Problem has been successfully copied */
 ...
 */
//just change the tolerances
int CVode_change_SStolerances(void *cvode_mem, realtype reltol, realtype abstol){
  CVodeMem cv_mem;
